...
::printguideparams <- function()
{
	...
	angle.z = 0
	angle = angle.Forward()
	text += format("Vector(%f, %f, %f), Vector(%f, %f, %f)", pos.x, pos.y, pos.z, angle.x, angle.y, angle.z)
...
::prodemoknight <- {
	...
	function OnGameEvent_player_say(params)
	{
		...
		{
			clean()
			IncludeScript("pro_demoknight_v2", ROOT)
	...
	version = "2.133"
	debug_state = false
	bot_tag_list = ["bot_pro_demoknight", "bot_hyper_bomb_carrier", "bot_pro_demoknight_homewrecker", "bot_pro_busterknight"]
	function assign(hBot, tag)
	{
		// printl("assign: " + hBot)

		//hBot.SetAbsOrigin(Vector(-654.165588, 1454.743042, -388.290680))
		//SetPropBool( hBot, "m_bGlowEnabled", true )
	...
	hasCheckedEnemiesOfTeamBlueThisTick = false
	hasCheckedEnemiesOfTeamRedThisTick = false
	CheckPhasingList = []
	...
	function Think()
	{
		...
		EntFire("tf_gamerules", "RunScriptCode", "prodemoknight.hasCheckedEnemiesOfTeamBlueThisTick = false")
		EntFire("tf_gamerules", "RunScriptCode", "prodemoknight.hasCheckedEnemiesOfTeamRedThisTick = false")
	...
	function TrimpInfoParser()
	{
        if ( !("trimp_spot_info" in ROOT) )
        {
            ::trimp_spot_info <- {}
        }

		foreach ( spot in ::trimp_spot_info )
		{
			...
			if ( !spot.rawin("approachInstruction") )
			{
				spot.approachInstruction <- null
			}

			if ( !spot.rawin("jump_extent") )
			{
				spot.jump_extent <- null
			}
...
enum State
{	//0						//3							            // 6					//8									//11
	IDLE, DEEPIDLE, CLEANUP, GOTOTRIMPSPOT, PIPEJUMP, TRIMPSTRAFE, AIRSTRAFE, AIRBORNE, SIMULATETRIMP, CIRCLESENTRY, CAMPTELE, BUSTER, DETONATING, CHARGEATTARGET, CHASE, AVOID, TEST
};
::ProDemoknightBot <- class
{
	hOwner 		= null
	hWeapon		= null
	hWeapon_primary = null
	is_loose_cannon   = false
	AI_Bot_myDemo	= null
	hateSentries	= false
	...
	constructor(hOwner, bot_type)
	{
		...
		this.hOwner.GetScriptScope().Think <- function()
		{
			...
			{
                if ( bot_type == "bot_hyper_bomb_carrier" )
                    return me.Think_HyperBombCarrier()
                else
				    return me.Think()
			}
			else
		...
		if ( bot_type == "bot_hyper_bomb_carrier" )
		{
		    //hOwner.PressFireButton(100)
            circumventSpeedCap = false
            hasCharge = true
        }
		...
		if ( circumventByAD43 )                                             //$rafmod
		{
			SetPropBool(hOwner, "m_bForcedSkin", true)
			SetPropInt(hOwner, "m_nForcedSkin", 1)
			if ( hOwner.GetCustomAttribute("receive friendly fire", 0) )	//$rafmod
			{
	...
	function circumventSpeedCap_func( state )
	{
		...
		if (state)
		{
			...
			if ( !circumventing )
			{
				...
				if ( !hasFriendlyFire )
				{
					hOwner.AddCustomAttribute("receive friendly fire", 1, 0)
				}
		...
		else
		{
			if ( circumventing )
			{
				...
				if ( !hasFriendlyFire )
				{
                    hOwner.RemoveCustomAttribute("receive friendly fire")
				}
	...
	function circumvent_timeout( timeout_for = 0.5 )
	{
		if ( circumvent_timeout_timer.IsValid() )
		{
	...
	function checkAABBCollide( targetWrapper, speed = 2000 )	// should look at IsBoxIntersectingBox() from source sdk
	{
		local targetOrigin
		local targetMaxs
		...
		{
			local mins		= prodemoknight.PlayerListInfo[targetWrapper].mins
			targetOrigin	= prodemoknight.PlayerListInfo[targetWrapper].origin + mins
			targetMaxs		= prodemoknight.PlayerListInfo[targetWrapper].maxs - mins
		}
	...
	function DoNotLetOtherDemosStopMe()
	{
		...
		foreach ( demo in prodemoknight.ListOfProDemoknights.values() )
		{
			...
			if ( (demo.AI_Bot_myDemo.cur_pos - AI_Bot_myDemo.cur_pos).LengthSqr() > 250000 )	// 500 * 500 hard coded
				continue
	...
	function DoNotPhaseTheOtherTeam()
	{
		...
		collectAllPlayers()
		// local EnemyList = [].extend( prodemoknight.PlayerList ).extend( prodemoknight.SentryList )
		foreach ( player in prodemoknight.PlayerList )
		{
			if ( player.GetTeam() == AI_Bot_myDemo.team
				...
				|| (prodemoknight.PlayerListInfo[player].origin - AI_Bot_myDemo.cur_pos).LengthSqr() > 250000	// 500 * 500 hard coded
				) continue
		...
		{
			if ( (getSentryPos(sentry) + Vector(0,0,sentry.GetBoundingMaxs().z) - AI_Bot_myDemo.cur_pos).LengthSqr() < 10000 )	// 100 * 100 hard coded
			{
				circumvent_timeout()
				return
			}
	...
	function changeState_HyperBombCarrier(new_state)
	{
		if ( current_state == new_state )
			return
		...
		for ( i = 0; i < 10; i++ )
		{
			...
			switch ( new_state )
			{
			    ...
			    case State.PIPEJUMP:
					pipe_list = []
					local pipe = null
                    while ( pipe = FindByClassname( pipe, "tf_projectile_pipe" ) )
     			    {
                        if ( GetPropEntity( pipe, "m_hThrower") == hOwner )
    				    {
                            pipe_list.append( pipe )
                        }
                    }
                    printl("pipe_list " + pipe_list.len())
					break
		    }
		    //------------------------------------------------
		    if ( evaluate_new_state == new_state )	// no more change, move on
	...
	function Think()
	{
		...
		if ( should_attack && !hateSentries )
		{
			if ( GetPropFloat(hWeapon, "m_flNextPrimaryAttack") < AI_Bot_myDemo.time )
			{
				meleeThisGuy = null
				// should_attack = false
			}
			...
			if ( !meleeThisGuy )
			{
				...
				{
					if ( (prodemoknight.PlayerListInfo[threat].origin - AI_Bot_myDemo.cur_pos).LengthSqr() > 1000000 ) continue
    ...
    my_pipe_age = 0
	function Think_HyperBombCarrier()
	{
	    if ( my_pipe && my_pipe.IsValid() )
		{
		    if ( !my_pipe_age )
			{
			    my_pipe_age = AI_Bot_myDemo.time
        ...
        {
            fuse_time = AI_Bot_myDemo.time - my_pipe_age
            //printl("my_pipe_age " + fuse_time)
		...
		if ( no_SUPPRESS_FIRE && !is_charging_cannon_timer.IsValid() )
		{
			no_SUPPRESS_FIRE = false
			hOwner.AddBotAttribute(SUPPRESS_FIRE)
		}
        ...
        if ( current_state == State.IDLE )
        {
		    changeState_HyperBombCarrier( State.DEEPIDLE )
            //return 1
        }
	    else if ( current_state == State.DEEPIDLE )     // ENUM_RENAMING: let the default TF2 AI controls me
		{
			// don't let me lose my charge
			SetPropFloat(hOwner, "m_Shared.m_flChargeMeter", 0)

			// check if I am holding the bomb
			if ( hasCharge && hOwner.HasItem() && !hOwner.InCond(51) )  // hOwner.HasItem() &&
			{
			    changeState_HyperBombCarrier( State.PIPEJUMP )
				hOwner.AddCustomAttribute("move speed bonus", 2, 0)
			}
		}
		else if ( current_state == State.PIPEJUMP )
		{
			PIPEJUMP_Think()
		}
		else if ( current_state == State.TRIMPSTRAFE )  // ENUM_RENAMING: using custom unlimited turn algo
		{
			FREETURNTRIMP_Think()
		}
	...
	function SwitchToPrimary()
	{
        ...
        if ( hOwner.GetActiveWeapon() != hWeapon_primary )
		{
			hOwner.Weapon_Switch( hWeapon_primary )
			switch_wait_time = AI_Bot_myDemo.time + 0.3
	    	return true
		}
	...
	function charge_cannon_on_self( charge_time )
	{
        SetPropFloat(hWeapon_primary, "m_flNextPrimaryAttack", AI_Bot_myDemo.time + 0.04)
        holdAttack( charge_time )
        printl("holding attack now")

        is_charging_cannon_timer    = CreateTimer(@() {}, charge_time)

	}

	fuse_time = 2
	time_to_pipe_detonate = 0
	switch_wait_time = 0
	my_pipe = null
	//is_in_no_pipe_phase = false
	pipe_list = null
	function PIPEJUMP_Think()
	{
		if ( switch_wait_time > AI_Bot_myDemo.time )
			return

        if ( is_loose_cannon )
        {
            if ( !is_charging_cannon_timer.IsValid() )
            {
                if ( !SwitchToPrimary() )
    			    changeState_HyperBombCarrier( State.IDLE )

                if ( hOwner.GetActiveWeapon() == hWeapon_primary )
                {
                    hOwner.SnapEyeAngles( QAngle(89, 0, 0) )
                    interface_trimpToGuide( trimp_forward_info.path_hill_with_cars )

                    charge_cannon_on_self( 1.2 )

                    local ref = this
                    SimpleCharge_Timer    = CreateTimer(function() {
                        ref.startCharge( true )
             			ref.changeState_HyperBombCarrier( State.TRIMPSTRAFE )
             			ref.lenient_TRIMPSTRAFE_timer = CreateTimer(@() ref.changeState_HyperBombCarrier( State.IDLE ), 0.2)

                            SetPropInt(ref.hOwner, "m_afButtonForced", IN_DUCK)
               	        ref.duck_timer = CreateTimer(@() SetPropInt(ref.hOwner, "m_afButtonForced", 0), 0.5)
                    }, 1.02)
                }
            }
            else
            {
                local dir = VectorAngles(GeneratePointWithIntersection() - AI_Bot_myDemo.cur_eye_pos)
                dir.y -= 54.1
                dir.x = 89
                CleanQAngle(dir)
        ...
        {
    		// I don't have a pipe in the map, make one
    		if ( !my_pipe || !my_pipe.IsValid() )
    		{
                ...
                local pipe = null
     			while ( pipe = FindByClassname( pipe, "tf_projectile_pipe" ) )
     			{
    				if ( GetPropEntity( pipe, "m_hThrower") == hOwner )
    				{
                        if ( pipe_list.find(pipe) != null)
                            continue

       					// fuse_time = 2.3 * 0.7 + AI_Bot_myDemo.time   // wiki numbers are always wrong
                        local actual_fuse_time = fuse_time * 10
                        if ( !(actual_fuse_time % 2))    // if divisible by 0.2
                        {
                            actual_fuse_time += 2
                        }
                        local tickInterval_10 = tickInterval * 10
                        actual_fuse_time = (((actual_fuse_time / (tickInterval_10)).tointeger() - 2) * tickInterval_10)/10
                        time_to_pipe_detonate = actual_fuse_time + AI_Bot_myDemo.time

                        //printl(actual_fuse_time)
    			...
    			if ( hOwner.GetActiveWeapon() == hWeapon_primary && GetPropFloat(hWeapon_primary, "m_flNextPrimaryAttack") < AI_Bot_myDemo.time )
    			{
    				hOwner.SnapEyeAngles( QAngle(89, 0, 0) )
    				startAttack()

    				// pick a guide here
    				interface_trimpToGuide( trimp_forward_info.path_partial_hill_then_stairs )

    				return
    			}
    		}
    		// now I do have a pipe, stand on it
    		else
    		{
                //is_in_no_pipe_phase = false
                SetPropFloat(hOwner, "m_Shared.m_flChargeMeter", 100)

    			if ( hOwner.GetActiveWeapon() != hWeapon )
    				hOwner.Weapon_Switch( hWeapon )

    			if ( current_guide )
    				AI_Bot_myDemo.SnapAt(current_guide[0].where)

    			local delta = time_to_pipe_detonate - AI_Bot_myDemo.time
    			local proj_vel = my_pipe.GetAbsVelocity()

    			local det_location = my_pipe.GetOrigin() + proj_vel * delta
    			if ( (det_location - AI_Bot_myDemo.cur_pos).LengthSqr() > 5 * 5 )
    			{
    				moveToSpot( det_location )
    			}

    			if ( delta < 0 )
    			{
    				startCharge( false )
    				changeState_HyperBombCarrier( State.TRIMPSTRAFE )

    				local ref = this
    				lenient_TRIMPSTRAFE_timer = CreateTimer(@() ref.changeState_HyperBombCarrier( State.IDLE ), 0.2)
	...
	OptimalAngle_FirstOptionBool = true
	vel_old = 0
	// vel_old_delay = 0
	// prev_vertical_vel = 0
	function FREETURNTRIMP_Think()
	{


		if ( following_guide )
		{
			local distance = ( AI_Bot_myDemo.cur_pos - current_guide_location.where )
			distance.z = 0
			if ( distance.Length() < 100 )
			{
				local index = current_guide.find(current_guide_location) + 1
				if ( index >= current_guide.len() )
				{
					changeState_HyperBombCarrier( State.IDLE )
				}
				else
				{
					//printl(index)
					current_guide_location = current_guide[index]
				}
			}
		}
		...
		if ( hOwner.InCond(17) )
		{
			...
			// printl("total vel: " + vel.Length())
			vel.z = 0
			local speed = vel.Length()
			// printl(speed)
			//printl("gain: " + (speed-vel_old) + " angle: " + AI_Bot_myDemo.cur_eye_ang.y)

			local texttoprintl = "gain: " + (speed-vel_old)
			local printthistime = false
			// if ( speed == vel_old )
			// {
				// printthistime = true
				// vel_old_delay = 1
			// }
			// else if ( vel_old_delay )
			// {
				// printthistime = true
				// vel_old_delay -= 1
			// }
			...
			if ( following_guide )
			{
				targetPos = GeneratePointWithIntersection()
				// FindNextRequirementOfNextPoint() // we doing it in above function

				//prodemoknight.CreateMarker()
				//prodemoknight.hMarker.SetAbsOrigin(targetPos)
			}
			else
			{
				targetPos = Vector(210.261017, 1361.614746, -127.968750)
			}


			if ( vel.Length() >= maxChargeSpeed - 1 )
			{
				// local cur_eye_ang = AI_Bot_myDemo.cur_eye_ang
				local cur_eye_ang = hOwner.EyeAngles()
				local pos = AI_Bot_myDemo.cur_pos + Vector()
				cur_eye_ang.x = 0

				local targetDir = targetPos - AI_Bot_myDemo.cur_pos
				local diffAngleVelTar = DiffQAngle( VectorAngles(vel), VectorAngles(targetDir) );

				// local current_speed = maxChargeSpeed - maxChargeSpeed * 10 * tickInterval	// ( - accel_speed )

				// move forward x tick
				local skip_x_ticks = 1
				// if ( !(speed-vel_old) && AI_Bot_myDemo.cur_pos.z > -432 )
					// skip_x_ticks = 2
				while ( skip_x_ticks )
				{
					skip_x_ticks -= 1

                    cur_eye_ang.y = CalculateAngleGeneral_FreeTurn( diffAngleVelTar, targetPos, pos, vel )

					local current_speed = vel.Dot(AI_Bot_myDemo.cur_eye_fwd)
					local add_speed = maxChargeSpeed - current_speed
					if ( add_speed > 0 )
					{
						local accel_speed = maxChargeSpeed * 10 * tickInterval
						if ( accel_speed > add_speed )
							accel_speed = add_speed

						vel += AI_Bot_myDemo.cur_eye_fwd * accel_speed;
						pos += vel * tickInterval;

						diffAngleVelTar = DiffQAngle( VectorAngles(vel), VectorAngles(targetDir) );
					}
				...
				// printl(OptimalAngle)
				// printl(" OptimalAngle : " + cur_eye_ang.y)

				// printl(maxChargeSpeed * 10 * tickInterval + " " + (maxChargeSpeed - vel.Dot(OptimalAngle)))


				texttoprintl += " angle: " + cur_eye_ang.y
				// texttoprintl += " angle: " + AI_Bot_myDemo.cur_eye_ang.y
				texttoprintl += " angle: " + diffAngleVelTar
				texttoprintl += " diff: " + DiffQAngle( VectorAngles(vel), cur_eye_ang );
				texttoprintl += " side: " + OptimalAngle_FirstOptionBool

				cur_eye_ang.y	= CalculateAngleGeneral_FreeTurn( diffAngleVelTar, targetPos, pos, vel )
				if ( is_charging_cannon_timer.IsValid() || (hOwner.GetFlags() & FL_ONGROUND) )
				    cur_eye_ang.x = 89
                ...
                if ( is_gazing )
                {
                    ...
                    {
                        AI_Bot_myDemo.SnapAt( gaze_at )
                        //prodemoknight.hMarker.SetAbsOrigin( gaze_at )

                        //local dir = VectorAngles(gaze_at - AI_Bot_myDemo.cur_eye_pos)
                        //printl("supposed angle:" + dir)
                        //me().SetAbsOrigin( gaze_at )
                        //ClientPrint(null, 3, format("\x079EC34F%s\x01", gaze_at.tostring()))
                    }
                ...
                {
    				// NetProps.SetPropVector(hOwner, "pl.v_angle", cur_eye_ang + Vector())
    				hOwner.SnapEyeAngles(cur_eye_ang)
                }

				// prodemoknight.simulated_look_best.append(QAngle(0, cur_eye_ang.y, 0))
				// prodemoknight.simulated_vel_best.append(vel)

				texttoprintl += "\n dif3: " + DiffQAngle( VectorAngles(vel), hOwner.EyeAngles() );
				// printl(texttoprintl)
			}
	...
	function CalculateAngleGeneral_FreeTurn( diffAngleVelTar, targetPos, pos, vel )
	{
        if ( current_node_requirement )
        {
            if ( current_node_requirement.use_cannon )
            {
                local distance = (current_node_requirement.where - pos)
                distance.z = 0
                local time_to_reach_target_xy = distance.Length() / vel.Length()

                if ( time_to_reach_target_xy < 1.15 && !is_charging_cannon_timer.IsValid() )
                {
                    charge_cannon_on_self(0.96)
                }
            }
            if ( current_node_requirement.max_height )
            {
                local time_to_reach_target_z = calcTimeToTargetZ(AI_Bot_myDemo.cur_pos.z, current_node_requirement.max_height, AI_Bot_myDemo.cur_vel.z, sv_gravity, true )

                local distance = (current_node_requirement.where - pos)
                distance.z = 0
                local time_to_reach_target_xy = distance.Length() / vel.Length()

                if ( !time_to_reach_target_z )
                {
                    // can't reach maximum height anyways
                    printl("can't reach")
                }
                else if ( time_to_reach_target_z[0] > time_to_reach_target_xy )   // the first one is on the way up
                {
                    // i can make it there before i hit the max_height
                    printl("reach before going down")
                }
                else if ( time_to_reach_target_z[1] > time_to_reach_target_xy )
                {
                    // use slow trimp algo here
                    printl("too fast")
                    printl("time_to_reach_target_z[1]: " + time_to_reach_target_z[1])
                    //printl("time_to_reach_target_z[0]: " + time_to_reach_target_z[0])
                    printl("time_to_reach_target_xy: " + time_to_reach_target_xy)
		            return CalculateMaxTurnNoSpeedLossAngle_FreeTurn( targetPos, pos, vel )
                }
            }
        }

		// if ( vel.Length() < 2000 )
		if ( abs(diffAngleVelTar) < 10 )
		    return CalculateOptimalAngle_FreeTurn( targetPos, pos, vel )
		//else if ( abs(diffAngleVelTar) < 20 )
		else
		    return CalculateMaxTurnNoSpeedLossAngle_FreeTurn( targetPos, pos, vel )
		//else
		//    return CalculateSharpTurn_FreeTurn( targetPos, pos, vel )
	}

    current_guide			= null
	current_guide_location	= null
	current_guide_node_index    = null
	current_node_requirement    = null
	following_guide = false
	function interface_trimpToGuide( guide )
	{
		current_guide	= guide
		following_guide = true

		current_guide_location = current_guide[0]
		current_guide_node_index    = 0
	}
    current_guide = null
 	PointRadius = 90
	function GeneratePointWithIntersection()
	{
		// if ( !current_guide )
		// {
		...
		Assert(current_guide, "hell naw")

		if ( PointRadius > 90 )
			PointRadius -= 3

		local list_len	= current_guide.len()
		// local radius	= PointRadius

		local result_index	= null
		local result	= []
		local result_t	= []
		...
		while (true)
		{
			...
			for (local i = 0; i < list_len - 1; i++)
			{
				result_t = circle_segment_intersections(current_guide[i].where, current_guide[i+1].where, AI_Bot_myDemo.cur_pos, PointRadius)
				if ( result_t.len() )
				{
					result		= result_t
					result_index	= i
			...
			{
				// prodemoknight.CreateMarker()
				// printl("radius " + PointRadius + " " + result_index)
				// foreach (h in result)
				    // printl("h " + h)
					// prodemoknight.hMarker.SetAbsOrigin(h)
				break
			}
		...
		//if ( result.len() > 1)
		//	printl("OKAY THIS IS MORE THAN 11111111")

		local max = 0
		local max_index = -1
		local length_check = 0
		for ( local i = 0; i < result.len(); i++ )
		{
			length_check = (current_guide[result_index].where - result[i]).Length()
			if ( length_check > max )
			{
				max = length_check
				max_index = i
        ...
        // calculate current distance along the guide line
        //local total_distance = 0
        //for ( local i = 0; i != result_index; i++ )
        //{
        //    total_distance += (current_guide[result_index] - current_guide[result_index+1])
        //}
        //total_distance += length_check

        //local total_distance = trimp_forward_info.cached_distances[current_guide][result_index]
        //total_distance += length_check

		//FindNextRequirementOfNextPoint( result_index )

		return result[max_index]
	}
    function FindNextRequirementOfNextPoint( current_node_index )
    {
        for (local i = 0; i < current_guide.len(); i++)
		{
		    local node = current_guide[i]
			if ( node.max_height )
			{
                if ( i > current_node_index )
                {
                    current_node_requirement = node
                    //printl("current_node_requirement: " + i)
                    return
                }
                else
                {
        ...
        current_node_requirement = null
        //printl("current_node_requirement: none")
    }

	function CalculateOptimalAngle_FreeTurn( targetPos, cur_pos, vel )
	{
		// local cur_eye_ang = AI_Bot_myDemo.cur_eye_ang
		...
		{
			local threshold = 3
			if ( diffAngleVelTar > threshold )
				OptimalAngle_FirstOptionBool = false
			else if ( diffAngleVelTar < -threshold )
				OptimalAngle_FirstOptionBool = true

			// printl( DiffQAngle( VectorAngles(vel), VectorAngles(OptimalAngle.B1) ) +  DiffQAngle( VectorAngles(vel), VectorAngles(OptimalAngle.B2) ) );

			// local combime = OptimalAngle.B1 + OptimalAngle.B2
			// combime.Norm()
			// local new_vel = (vel + Vector())
			// new_vel.Norm()
			// printl( combime.Dot(new_vel) )

			if ( OptimalAngle_FirstOptionBool )
		...
		{
			OptimalAngle = targetDir
			printl("this is a problem")
		}
		Assert(OptimalAngle, "oops")
		// printl(OptimalAngle)
		local angle = VectorAngles( OptimalAngle ).y
		if ( angle > 180 )
			angle -= 360

		return angle
	}
	function CalculateMaxTurnNoSpeedLossAngle_FreeTurn( targetPos, cur_pos, vel )
	{

		local targetDir = targetPos - cur_pos
		local diffAngleVelTar = DiffQAngle( VectorAngles(vel), VectorAngles(targetDir) );

		local maxAccelPerTick = maxChargeSpeed * 10 * tickInterval	// ( - accel_speed )

		local OptimalAngle = WhenYouNeedTheSameVectorDiffAngle( vel, maxAccelPerTick )
		if ( OptimalAngle )
		{
			local threshold = 0.1
	...
	function CalculateSharpTurn_FreeTurn( targetPos, cur_pos, vel )
	{
	    local targetDir = targetPos - cur_pos
		local diffAngleVelTar = DiffQAngle( VectorAngles(vel), VectorAngles(targetDir) );

        local OptimalAngle = {B1 = Vector(-vel.y, vel.x, 0), B2 = Vector(vel.y, -vel.x, 0)}	// 90* apart
        if ( diffAngleVelTar > 0 )
            OptimalAngle = OptimalAngle.B2
        else
            OptimalAngle = OptimalAngle.B1

        return VectorAngles( OptimalAngle ).y
	}

    gaze_at = null
    is_gazing = false
    gaze_count = 0
    before_gaze_at_timer = CreateTimer(@() {}, 0.1)
    ground_hit_swing_time = 0
	function PredictTouchingGround()
	{
	    //printl("dir.x = " + AI_Bot_myDemo.cur_eye_ang.x)

        local corners = [Vector(), Vector()]

        // direction we are moving in
        // then find corners that will touch the ground
        local dir = AI_Bot_myDemo.cur_vel * 1
        ...
        foreach ( corner in corners )
        {
            local time = swingTime + 0.005
            local start_pos = AI_Bot_myDemo.cur_pos + corner
            local end_pos = start_pos + AI_Bot_myDemo.cur_vel * time

            local extra_distance_caused_by_gravity = sv_gravity / 2 * time * time // 1/2*𝑎*𝑡^2
            end_pos.z += extra_distance_caused_by_gravity
    		...
    		if ( trace.hit )
            {
                local dir_to_hit = trace.pos - start_pos
                // check if i can actually hit it
                //local extrapolated_eye_pos = (AI_Bot_myDemo.cur_eye_pos + dir_to_hit)

                // i think the way this works, it always look at one corner, so we get the same angle everytime
                local eye_to_corner_dir = start_pos - AI_Bot_myDemo.cur_eye_pos

                local hit = false
                local trace_length = eye_to_corner_dir.Length()
                local melee_range = 128     // because I can swing while charging my range is increased
                if ( trace_length <= melee_range + melee_bounding_box / 2  )
                {
                    // it's a hit
                    hit = true
                }
                else if ( false )
                {
                    ...
                    eye_to_corner_dir.Norm()
                    local remaining = trace_length - meleeRange
                    local melee_line_trace_end_point = trace.pos + (eye_to_corner_dir * -remaining)

                    local hull_half_size = melee_bounding_box / 2
                    local hull_maxs = Vector(hull_half_size, hull_half_size, hull_half_size)
                    local trace_hull = {
             			start  = melee_line_trace_end_point,
             			end    = melee_line_trace_end_point,
                        hullmin = hull_maxs * -1
                        hullmax = hull_maxs
             			mask   = MASK_OPAQUE,
             			ignore = hOwner
              		}
              		TraceLineEx(trace_hull)

                    hit = trace_hull.hit
                }

                if ( hit && ground_hit_swing_time + 0.3 < AI_Bot_myDemo.time )
                {
                    if ( startAttack() )
                    {
                        ground_hit_swing_time = AI_Bot_myDemo.time
                        gaze_count = 1

                        local ref = this
                        before_gaze_at_timer = CreateTimer(@() ref.is_gazing = true, 0.2 )
                        //printl("ground_hit_swing_time: " + ground_hit_swing_time)
                    }
        ...
        //if ( ground_hit_swing_time + 0.19 < AI_Bot_myDemo.time  && ground_hit_swing_time + 0.21 > AI_Bot_myDemo.time )
	...
	function checkStuck2()
	{
		...
		if ( stuck_check_2_timeout < AI_Bot_myDemo.time )
		{
			...
			stuck_check_2_timeout = AI_Bot_myDemo.time + 2
		}
	}
	...
	function canChargeToTarget( target )
	{
		...
		if ( (hOwner.GetFlags() & (FL_ONGROUND | FL_DUCKING)) == (FL_ONGROUND | FL_DUCKING) )
		{
            return false
		}
	...
	function IsTargetCloseEnoughInT( target, timeleft = null, targetRange = 0 )
	{
		...
		local toTarget = prodemoknight.PlayerListInfo[target].origin - AI_Bot_myDemo.cur_pos
		toTarget.Norm()
		local t = timeleft

		local targetHalfBounding = prodemoknight.PlayerListInfo[target].maxs.x
		local targetPos = prodemoknight.PlayerListInfo[target].origin * 1
		local targetVel = target.GetAbsVelocity()
		local offset	= prodemoknight.PlayerListInfo[target].maxs.z - targetHalfBounding
		targetPos.z		+= offset
	...
	function moveToSpot(spot, simple_approach = false)
	{
		...
		local cur_speed_sqr = AI_Bot_myDemo.cur_speed * AI_Bot_myDemo.cur_speed

		// local scale = 1
		// if ( bot_scale != 1 )
			// scale *= bot_scale * bot_scale * 2.5

		if ( cur_speed_sqr > 200 && (AI_Bot_myDemo.cur_pos - targetPos).LengthSqr() < 1024 )	// 32 * 32 * scale
		{
			return
		}
		...
		if ( cur_speed_sqr > 100)
		{
			...
			{
				simple_approach_func(simple_approach, spot.jump_extent)
				// printl("move simple")
			}
		...
		else
		{
			...
			{
				// every 0.5s
				local alternate = ( (AI_Bot_myDemo.time - moving_time) / 0.5 ).tointeger() % 2 == 0
				...
				if ( !alternate )
				{
					...
					AI_Bot_myDemo.locomotion.Approach(randompos, 999)
					//printl("move random")
				}
				else
				{
	...
	function holdAttack( duration )
	{
		...
		no_SUPPRESS_FIRE = true
		hOwner.RemoveBotAttribute(SUPPRESS_FIRE)
		hOwner.PressFireButton( duration )
	...
	function useTrimpSpot(spot)
	{
		if ( !arrived )
		{
			...
			if ( !hateSentries )
			{
				closestThreat = AI_Bot_myDemo.FindClosestThreat(160000, false)
			}
			else
			{
				collectSentries()
				closestThreat = FindClosestSentry(160000)	// 400 * 400
			}
			if ( closestThreat )
			{
				changeState( State.CHASE )
			...
			moveToSpot( spot , spot.approachInstruction )
			///

			if ( !hasCharge || spot.occupied )
				return false
	...
	function TRIMPSTRAFE_Think()
	{
		...
		if ( hOwner.InCond(17) )
		{
			...
			if ( vel.Length() >= maxChargeSpeed - 1 || ( current_trimpSpot && current_trimpSpot.simplejump) )
			{
	...
	{
		if ( (prodemoknight.hasCheckedEnemiesOfTeamBlueThisTick && AI_Bot_myDemo.team == 3)
			|| (prodemoknight.hasCheckedEnemiesOfTeamRedThisTick && AI_Bot_myDemo.team == 2)
			)
			return

		if ( AI_Bot_myDemo.team == 3 )
			prodemoknight.hasCheckedEnemiesOfTeamBlueThisTick = true
		else if ( AI_Bot_myDemo.team == 2 )
			prodemoknight.hasCheckedEnemiesOfTeamRedThisTick = true
		else
			return
		...
		local my_list = [].extend( prodemoknight.PlayerList ).extend( prodemoknight.SentryList )
		foreach ( hPlayer in my_list )
		{
			if ( !hPlayer.IsValid()
				|| hPlayer.GetTeam() == AI_Bot_myDemo.team
			) continue

			if ( prodemoknight.PlayerListInfo.rawin(hPlayer) )
			{
				prodemoknight.PlayerListInfo[hPlayer].origin = getSentryPos(hPlayer)
			}
			else
			{
				local table = {}
				table.origin <- getSentryPos(hPlayer)
				table.mins <- hPlayer.GetBoundingMins()
				table.maxs <- hPlayer.GetBoundingMaxs()

				prodemoknight.PlayerListInfo[hPlayer] <- table
			}
		}
	}
	...
	function AVOID_Think()
	{
		...
		local backward = (AI_Bot_myDemo.cur_pos - prodemoknight.PlayerListInfo[targetAvoid].origin)
	...
	function CHASE_Think()
	{
		...
		if ( closestThreat )
			closestThreatPos = prodemoknight.PlayerListInfo[closestThreat].origin * 1
		...
		else
		{
			...
			{	//
				...
				{
					// printl("attacking")
					local actualRange = meleeRange + prodemoknight.PlayerListInfo[closestThreat].maxs.x + 36
	...
	function GetMaxChargeSpeed()		// also fetch turn rate and other stuff
	{
		...
		for (local i = 0; i < GetPropArraySize(hOwner, "m_hMyWeapons"); i++)
		{
			...
			if ( weapon )
			{
				...
				else if ( weapon.GetClassname() == "tf_weapon_grenadelauncher" )
				{
					hWeapon_primary = weapon
					is_loose_cannon = false
					hOwner.ClearAllWeaponRestrictions()
				}
				else if ( weapon.GetClassname() == "tf_weapon_cannon" )
				{
                    hWeapon_primary = weapon
					is_loose_cannon = true
					hOwner.ClearAllWeaponRestrictions()
				}
			}
		...
		if ( hWeapon_primary )
		{
		    fuse_time *= hWeapon_primary.GetAttribute("fuse bonus", 1 )
		}
	...
	function cleanup()
	{
		...
		try
		{
			...
			if ( is_charging_cannon_timer.IsValid() )
				is_charging_cannon_timer.Destroy()
			if ( duck_timer.IsValid() )
				duck_timer.Destroy()
			if ( before_gaze_at_timer.IsValid() )
				before_gaze_at_timer.Destroy()
...
::AI_Bot_ProDemoknight <- class {
	...
	function GetThreatDistanceSqr(target) {
		return (prodemoknight.PlayerListInfo[target].origin - cur_pos).LengthSqr()
	}
	function GetThreatDistanceSqrFromPoint(target, point) {
		return (prodemoknight.PlayerListInfo[target].origin - point).LengthSqr()
	}
	...
	function UpdatePathAndMove(target_pos, advanced = false, check_clear = false)
	{
		...
		if (path_recompute_time < time || check_clear) {
			...
			if ( ( this.team == TF_TEAM_RED && area_end.HasAttributeTF( TF_NAV_SPAWN_ROOM_BLUE ) ) ||
				 ( this.team == TF_TEAM_BLUE && area_end.HasAttributeTF( TF_NAV_SPAWN_ROOM_RED ) ) )
			{

	...
	*/
	...
	function ClampHopDistance(ar1, ar2, limit) 
	{
		...
		for (local i = 1.0; i <= 9.0; i++)
		{
			...
			local point = s_orig + ((e_orig - s_orig) * (i / 10.0))
			...
			local groundarea = NavMesh.GetNavArea(ground + Vector(0, 0, 10), 100)
	...
	function DebugPathTestSetPos(picker)
	{
		...
		EntFireByHandle(picker, "RunScriptCode", "self.SetHealth(0); self.TakeDamage(10000.0, 64, null)", 0.5, null, null)
		...
		OnGameEvent_teamplay_flag_event = function(params)
		{
			...
			if (found)
			{
				...
				{					
					...
					if (debug) DebugPathTestSetPos(EntIndexToHScript(params.player))

...
::SpawnHook <-
{
	...
	{
		local inflictor = params.attacker;
		local victim = params.const_entity;
		if (victim == null || inflictor == null || !victim.IsValid() || !inflictor.IsValid())
		{
			return;
		}
		...
		if (!inflictor.IsPlayer() || !inflictor.IsBotOfType(1337))
		{
			return;
		}

		local tags = {}
		inflictor.GetAllBotTags(tags)
		foreach (tag in tags)
		{
			if (tag.find("burn_on_hit") != null)
			{
				local params = split(tag, "|")
				local duration = 10.0
				if (params.len() >= 2)
				{
					duration = params[1].tofloat();
				}
				local ignitor = SpawnEntityFromTable("trigger_ignite", {
						burn_duration = duration,
						damage = 0.0,
						spawnflags = 1
				})
				EntFireByHandle(ignitor, "StartTouch", "", -1, victim, victim)
				EntFireByHandle(ignitor, "EndTouch", "", 0.015, victim, victim)
				EntFireByHandle(ignitor, "Kill", "", 0.015, null, null)
			}
		}
	}

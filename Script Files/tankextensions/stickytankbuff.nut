local STICKYTANKBUFF_VALUES_TABLE = {
	STICKYTANKBUFF_TURRET_MODEL             = "models/props_frontline/tank_turret.mdl"
	STICKYTANKBUFF_SND_SHOOT_CRIT           = "weapons/stickybomblauncher_shoot_crit.wav"
	STICKYTANKBUFF_SND_SHOOT                = "weapons/stickybomblauncher_shoot.wav"
	STICKYTANKBUFF_PROJECTILE_MODEL         = "models/weapons/w_models/w_stickybomb.mdl"
	STICKYTANKBUFF_PROJECTILE_SPREAD        = 25
	STICKYTANKBUFF_PROJECTILE_SPLASH_RADIUS = 189
	STICKYTANKBUFF_PROJECTILE_SPEED         = 2000
	STICKYTANKBUFF_PROJECTILE_DAMAGE        = 105
}
foreach(k,v in STICKYTANKBUFF_VALUES_TABLE)
...
TankExt.NewTankScript("stickytankbuff", {
	OnSpawn = function(hTank, sName, hPath)
	{
		local bBlueTeam = hTank.GetTeam() == 3
		local hTank_scope = hTank.GetScriptScope()
		local hModel1 = SpawnEntityFromTable("prop_dynamic", { origin = "-16 -66 108", angles = "-10 0 -90", model = STICKYTANK_TURRET_MODEL, skin = bBlueTeam ? 2 : 0 })
		local hModel2 = SpawnEntityFromTable("prop_dynamic", { origin = "-16 66 108", angles = "-10 0 90", model = STICKYTANK_TURRET_MODEL, skin = bBlueTeam ? 2 : 0 })
		hTank_scope.hMimic1 <- SpawnEntityFromTable("tf_point_weapon_mimic", { origin = "70 57 135", angles = "-10 0 0", damage = STICKYTANK_PROJECTILE_DAMAGE, modelscale = 1, modeloverride = STICKYTANK_PROJECTILE_MODEL, speedmax = STICKYTANK_PROJECTILE_SPEED, speedmin = STICKYTANK_PROJECTILE_SPEED, splashradius = STICKYTANK_PROJECTILE_SPLASH_RADIUS, weapontype = 3, spreadangle = STICKYTANK_PROJECTILE_SPREAD })
		hTank_scope.hMimic2 <- SpawnEntityFromTable("tf_point_weapon_mimic", { origin = "70 -57 135", angles = "-10 0 0", damage = STICKYTANK_PROJECTILE_DAMAGE, modelscale = 1, modeloverride = STICKYTANK_PROJECTILE_MODEL, speedmax = STICKYTANK_PROJECTILE_SPEED, speedmin = STICKYTANK_PROJECTILE_SPEED, splashradius = STICKYTANK_PROJECTILE_SPLASH_RADIUS, weapontype = 3, spreadangle = STICKYTANK_PROJECTILE_SPREAD })
		...
		hTank_scope.StickyThink <- function()
		{
			...
			if(Time() >= flTimeNext)
			{
				...
				EntFireByHandle(hMimic1, "DetonateStickies", null, 4, null, null)
				EntFireByHandle(hMimic2, "DetonateStickies", null, 4, null, null)
			}
			return -1
		}

...
::MerasmusThink <- function () 
{
	...
	local disguisemult = 1

	if ("__missionmodifier" in getroottable()) 
	{
		if (__missionmodifier.mod_active_double_robot_health == 1) disguisemult = 2
		if (__missionmodifier.mod_active_double_robot_health == 2) disguisemult = 3
		if (__missionmodifier.mod_active_double_robot_health == 3) disguisemult = 4
	}
    ...
    if (health <= maxhealth / 1.5 && phase == 0 && !self.InCond(65) && !self.InCond(87))
    {
        if (!self.HasBotTag("bot_transform") && anim == null && is_disguising == 0)
        {
            ...
            MerasmusNamespace.disguise_boss_health = 15000 * disguisemult
           ...
           // weapon.AddAttribute("health regen", 5, -1)
			...
			if ("__missionmodifier" in getroottable()) 
			{
				if (__missionmodifier.mod_active_double_robot_health == 1) weapon.AddAttribute("max health additive bonus", BASE_MAXHEALTH, -1)
				if (__missionmodifier.mod_active_double_robot_health == 2) weapon.AddAttribute("max health additive bonus", BASE_MAXHEALTH * 2, -1)
				if (__missionmodifier.mod_active_double_robot_health == 3) weapon.AddAttribute("max health additive bonus", BASE_MAXHEALTH * 3, -1)
			}
    ...
    else if (health <= maxhealth / 2 && phase == 1 && !self.InCond(65) && !self.InCond(87))
    {
        if (!self.HasBotTag("bot_transform") && anim == null && is_disguising == 0)
        {
            ...
            self.AddCustomAttribute("dmg taken increased", 0, 2) 
            self.AddBotTag("bot_transform")
            MerasmusNamespace.disguise_boss_health = 17500 * disguisemult
    ...
    function BossSetup() 
    {
        PopExt.AddRobotTag("bot_merasmus", 
        {
            OnSpawn = function(bot, tag) 
            {
				...
				if ("__missionmodifier" in getroottable()) 
				{
					local current_maxhealth = bot.GetMaxHealth()
					BASE_MAXHEALTH = bot.GetMaxHealth()
					if (__missionmodifier.mod_active_double_robot_health == 1) {
						for (local i = 0; i < 7; i++) {
							local wep = NetProps.GetPropEntityArray(bot, "m_hMyWeapons", i)
							if ( wep == null) continue

							wep.AddAttribute("max health additive bonus", current_maxhealth, -1)
							wep.ReapplyProvision()
							break
						}
						bot.SetHealth(current_maxhealth * 2)
					}
					if (__missionmodifier.mod_active_double_robot_health == 2) {
						for (local i = 0; i < 7; i++) {
							local wep = NetProps.GetPropEntityArray(bot, "m_hMyWeapons", i)
							if ( wep == null) continue

							wep.AddAttribute("max health additive bonus", current_maxhealth * 2, -1)
							wep.ReapplyProvision()
							break
						}
						bot.SetHealth(current_maxhealth * 4)
					}
					if (__missionmodifier.mod_active_double_robot_health == 3) {
						for (local i = 0; i < 7; i++) {
							local wep = NetProps.GetPropEntityArray(bot, "m_hMyWeapons", i)
							if ( wep == null) continue

							wep.AddAttribute("max health additive bonus", current_maxhealth * 3, -1)
							wep.ReapplyProvision()
							break
						}
						bot.SetHealth(current_maxhealth * 4)
                ...
                EntFireByHandle(bot, "CallScriptFunction", "SpawnMerasmus", 0.02, bot, bot)
            }
        })
            ...
            if (victim.IsBotOfType(TF_BOT_TYPE))
            {
                ...
                if (victim.HasBotTag("bot_transform"))
                {
                    ...
                    if (disguise_boss_health <= 0)
                    {
						...
						{
							if (__missionmodifier.mod_active_double_robot_health == 1) {
								victim.AddCustomAttribute("max health additive bonus", BASE_MAXHEALTH, -1)
							}
							if (__missionmodifier.mod_active_double_robot_health == 2) {
								victim.AddCustomAttribute("max health additive bonus", BASE_MAXHEALTH * 2, -1)
							}
							if (__missionmodifier.mod_active_double_robot_health == 3) {
								victim.AddCustomAttribute("max health additive bonus", BASE_MAXHEALTH * 3, -1)
							}
						}

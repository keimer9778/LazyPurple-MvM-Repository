...
::prosniper <- {
	...
	function test(int)
	{
		//NetProps.SetPropVector(findRandomPlayer(), "pl.v_angle", VectorAngles(birded.ListOfBirds[0].hOwner.GetBoneOrigin(int) - findRandomPlayer().EyePosition()) + Vector())
		
		
	}
	function mission(int, bool = true)
	{
		ListOfProSnipers[0].setmission(int, bool)
	}
...
::ProSniperBot <- class 
{
	...
	function setmission(int, bool = true)
		hOwner.SetMission(int, bool)
